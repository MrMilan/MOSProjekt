...
package Tradiator
  model pokojikSKotlem
    ...
    Buildings.Fluid.HeatExchangers.Radiators.RadiatorEN442_2 rad1(redeclare
        package Medium = MediumW, T_a_nominal = T_a_nominal, T_b_nominal = T_b_nominal, Q_flow_nominal = Q_flow_nominal, TAir_nominal = TRoo, energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial)
    ...
    Buildings.Fluid.Storage.ExpansionVessel exp(
      redeclare package Medium = MediumW,
      V_start=1,
      p=100000)                                 annotation(Placement(visible = true, transformation(origin = {36, -36}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Buildings.Fluid.FixedResistances.FixedResistanceDpM res2(
      redeclare package Medium = MediumW,
      m_flow_nominal=m_flow_nominal,
      dp_nominal=dp_nominal)                                         annotation(Placement(visible = true, transformation(origin = {-38, -62}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));

    Buildings.Fluid.HeatExchangers.HeaterCooler_u hea(
      redeclare package Medium = MediumW,
      m_flow_nominal=m_flow_nominal,
      dp_nominal=dp_nominal,
      Q_flow_nominal=Q_flow_nominal)                             annotation(Placement(visible = true, transformation(origin = {-4, -62}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));

    Modelica.Blocks.Sources.Constant dp(k=36)     annotation(Placement(visible = true, transformation(origin = {-76, 46}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant T(k=600)    annotation(Placement(visible = true, transformation(origin = {-16, -32}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant TSet(k=1)      annotation(Placement(visible = true, transformation(origin = {-46, 82}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  equation
    ...
    connect(T.y, hea.u) annotation(Line(points={{-5,-32},{16,-32},{16,-56},{8,-56}},                    color = {0, 0, 127}));
    connect(hea.port_b, res2.port_a) annotation(Line(points={{-14,-62},{-28,-62}},                              color = {0, 127, 255}));
    ...
    connect(TSet.y, val.y) annotation (Line(points={{-35,82},{-30,82},{-30,52},{-30,
            20}}, color={0,0,127}));
    connect(hea.port_a, del.ports[1]) annotation (Line(points={{6,-62},{38.6667,
            -62}},                                                                     color={0,127,255}));
    connect(exp.port_a, del.ports[2]) annotation (Line(points={{36,-46},{36,-62}}, color={0,127,255}));
    connect(res1.port_b, del.ports[3])
      annotation (Line(points={{84,8},{88,8},{88,6},{88,-62},{33.3333,-62}}, color={0,127,255}));
  ...
  model pokojSregulaci
    ...
    Buildings.Fluid.Sensors.TemperatureTwoPort temSen(redeclare package Medium
        =                                                                        Medium2, m_flow_nominal = m2_flow_nominal) annotation(Placement(visible = true, transformation(extent = {{4, -6}, {-16, 14}}, rotation = 0)));
    Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage val(redeclare
        package Medium =                                                                  Medium1, m_flow_nominal = m1_flow_nominal, dpValve_nominal = 6000)
      "Valve model"                                                                                                     annotation(Placement(visible = true, transformation(extent = {{14, 34}, {34, 54}}, rotation = 0)));
    Modelica.Blocks.Sources.TimeTable TSet(table = [0, 288.15; 600, 288.15; 600, 298.15; 1200, 298.15; 1800, 283.15; 2400, 283.15; 2400, 288.15])
      "Setpoint temperature"                                                                                                     annotation(Placement(visible = true, transformation(extent = {{-56, 74}, {-36, 94}}, rotation = 0)));
    Buildings.Fluid.HeatExchangers.WetCoilCounterFlow hex(redeclare package
        Medium1 =                                                                     Medium1, redeclare
    ...
    Buildings.Controls.Continuous.LimPID con(Td = 1, reverseAction = true, yMin = 0, controllerType = Modelica.Blocks.Types.SimpleController.PI, k = 0.1, Ti = 60)
      "Controller"                                                                                                     annotation(Placement(visible = true, transformation(extent = {{-16, 74}, {4, 94}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp TWat(height = 30, offset = T_a1_nominal, startTime = 300, duration = 2000)
      "Water temperature, raised to high value at t=3000 s"                                                                                                     annotation(Placement(visible = true, transformation(extent = {{-96, 38}, {-76, 58}}, rotation = 0)));
    Buildings.Fluid.FixedResistances.FixedResistanceDpM res(from_dp = true, redeclare
        package Medium =                                                                               Medium2, dp_nominal = 100, m_flow_nominal = m2_flow_nominal) annotation(Placement(visible = true, transformation(origin = {-44, 0}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Buildings.Fluid.FixedResistances.FixedResistanceDpM res1(from_dp = true, redeclare
        package Medium =                                                                                Medium1, dp_nominal = 3000, m_flow_nominal = m1_flow_nominal) annotation(Placement(visible = true, transformation(origin = {84, 44}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
  ...
  equation
    connect(pump1.port_b, tank.ports[2]) annotation(Line(points={{-44,-16},{-74,
            -16},{-74,38},{-72,38}},                                                                              color = {0, 127, 255}));
    ...
    connect(thermalConductor1.port_a, heater.heatPorts[1]) annotation(Line(points={{-46,42},
            {-46,42},{-46,32.4},{-47.45,32.4}},                                                                                   color = {191, 0, 0}));
    connect(room.port_b, radiator.heatPorts[1]) annotation(Line(points={{26,42},
            {26,38},{26,32.4},{24.55,32.4}},                                                   color = {191, 0, 0}));
